...
export type InspectionType = 'outbound' | 'returns' | 'retag';
...
export interface Suggestable<T> {
  value: T | null;
  source: 'manual' | 'ml-accepted' | 'ml-edited' | 'empty';
  mlSuggestion?: T;
  mlConfidence?: number;
  mlVersion?: string;
  mlAnalyzedAt?: string;
...
export interface PhotoMLResults {
  bagCount?: number;
  bagCountConfidence?: number;
  detectedBatchCode?: string;
  batchCodeConfidence?: number;
  accuracyLabelDetected?: boolean;
  // Stop sticker detection — usually on a side photo
  detectedStopNumber?: number;
  stopStickerConfidence?: number;
  rawText?: string;
  picklistFields?: PicklistExtractedFields;
  bolFields?: BOLExtractedFields;
  placardFields?: PlacardExtractedFields;
  returnsBolFields?: ReturnsBOLExtractedFields;
...
export interface OCRDebugInfo {
  // Original OCR text before any post-processing
  rawText?: string;
  // Per-word output from Tesseract, sorted by confidence
  words?: Array<{ text: string; confidence: number }>;
  // Overall Tesseract confidence for the page
  ocrConfidence?: number;
  // How long Tesseract took
  durationMs?: number;
  // Candidates that matched a known batch-code format pattern
  formatValidatedCandidates?: Array<{
    text: string;
    confidence: number;
    matchedPattern?: string;
  }>;
  // What we were trying to match against (from picklist)
  expectedBatches?: string[];
  // Best match against expected (if any)
  matchedAgainstExpected?: boolean;
  editDistance?: number;
  matchConfidence?: 'high' | 'medium' | 'low';
}

export interface PicklistExtractedFields {
  loadNumber?: string;
  shipDate?: string;
  carrier?: string;
...
export interface BOLExtractedFields {
  loadNumber?: string;
  shipDate?: string;
  carrier?: string;
...
export interface PlacardExtractedFields {
  batches?: PlacardBatch[];
  confidence?: number;
}

export interface PlacardBatch {
  batchCode?: string;
...
export interface ReturnsBOLExtractedFields {
  bolNumber?: string;
  receivedDate?: string;
  ...
  expectedBaggedProduct?: number;
  confidence?: number;
}

export interface ReturnsDamageAssessment {
...
export interface MLTrainingFlag {
  flaggedAt: string;
  flaggedBy: string;
  hint?: 'ocr_failed' | 'low_confidence' | 'unusual_angle' | 'unusual_lighting' | 'other';
  notes?: string;
}
